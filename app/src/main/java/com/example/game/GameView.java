...
public class GameView extends View {
    ...
    private Runnable r;
    private ArrayList<Plane> arrPipes;
    ...
    private void initPipe() {
        ...
        for(int i = 0 ; i< sumpipe/2; i++){
            if(i<sumpipe/2){
                this.arrPipes.add(new Plane(Constants.SCREEN_WIDTH +i * ((Constants.SCREEN_WIDTH+200*Constants.SCREEN_WIDTH/1080) /(sumpipe/2)),
                        0, 200*Constants.SCREEN_WIDTH/1080 ,Constants.SCREEN_HEIGHT/2));
                this.arrPipes.get(this.arrPipes.size() -1).setBm(BitmapFactory.decodeResource(this.getResources(), R.drawable.ucak2));
                this.arrPipes.get(this.arrPipes.size() -1).randomY();
            }else{
                this.arrPipes.add(new Plane(this.arrPipes.get(i - sumpipe/2).getX(), this.arrPipes.get(i - sumpipe/2).getY()
                        +this.arrPipes.get(i - sumpipe/2).getHeight() + this.distance, 200*Constants.SCREEN_WIDTH/1080, Constants.SCREEN_HEIGHT/2));
                this.arrPipes.get(this.arrPipes.size() -1).setBm(BitmapFactory.decodeResource(this.getResources(), R.drawable.ucak1));
            }
        }
    ...
    private void initBird() {
        ...
        bird.setY(100 * Constants.SCREEN_HEIGHT / 2 - bird.getHeight() / 2);
